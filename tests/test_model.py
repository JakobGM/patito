...
def test_model_properties_after_classmethod():
    class ModelWithConstraints(pt.Model):
        int_col: int = pt.Field(
            dtype=pl.UInt64, ge=0, lt=100, constraints=pt.field.sum() == 100
        )
        float_col: float
        string_col: str = "default_strings"
        list_col: list
        liststr_col: list[str]
        optlist_col: Optional[list[str]]
        optint_col: Optional[int]
        date_col: date = pt.Field(dtype=pl.Date)
        datetime_col: datetime = pt.Field(dtype=pl.Datetime)
    ...
    properties_after_with_fields = (
        ModelWithConstraints.with_fields()._schema_properties()
    )
    properties_after_rename = ModelWithConstraints.rename({})._schema_properties()
    properties_after_select = ModelWithConstraints.select(
        [
            "int_col",
            "float_col",
            "string_col",
